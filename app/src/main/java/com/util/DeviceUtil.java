...
//        tmpRate = phoneHeight / SCREEN_BASE_HEIGHT;
    ...
    public static String getDeviceIdObtain(Context Ctx) {
        ...
        try {
            ...
            DebugUtil.showDebug("usim address-------->" + DID);
            manager = null;
        } catch (Exception err) {
            DID = "";
        }
...
//                wm = null;
                ...
                DID = "";
                DebugUtil.showDebug("mac address-------->" + err.toString());
            }
            if (DID == null)
                DID = "";
            // MAC Address 에 구분자 제거
            DID = DID.replace(":", "");
            DebugUtil.showDebug("mac address-------->" + DID);
        }
        if (DID == null)
            DID = "";
...
//	
    ...
    public static void saveClipboard(Context context, String text) {
        if (getSdkVersion() >= Build.VERSION_CODES.HONEYCOMB) {
            ...
            DebugUtil.showToast(context, "카카오톡 ID가 클립보드에 복사되었습니다!");
        }
